...
/*
 ...
 //
 // Written by Jan Wigginton, c++ porting by F.Rosa (2010)
 */

void getSNPHWE(double &p_hwe, int obs_hets, int obs_hom1, int obs_hom2) {
	if (obs_hom1 < 0 || obs_hom2 < 0 || obs_hets < 0) {
		cout <<	"FATAL ERROR - SNP-HWE: Current genotype configuration (" << obs_hets << obs_hom1 << obs_hom2 << ") includes a negative count";
		p_hwe = 1.0;
	...
	if (genotypes == 0) {
		cout << "FATAL ERROR - SNP-HWE: Current genotype configuration ("<< obs_hets << obs_hom1 << obs_hom2 << ") includes zero count genotype" ;
		p_hwe = 1.0;
		return;	//exit(EXIT_FAILURE);
	}
	...
	if (het_probs == NULL) {
		cout <<	"FATAL ERROR - SNP-HWE: Unable to allocate array for heterozygote probabilities";
		p_hwe = 1.0;
		return;	//exit(EXIT_FAILURE);
	}
	...
	for (curr_hets = mid; curr_hets > 1; curr_hets -= 2) {
		het_probs[curr_hets - 2] = het_probs[curr_hets] * curr_hets
				* (curr_hets - 1.0) / (4.0 * (curr_homr + 1.0) * (curr_homc
				+ 1.0));
